...
const renderShoeItems = (shoes) => {
  ...
  for(let i = 0; i < shoes.length; i++){
    ...
    shoeItems += `<div class="shoeItem" data-id="${shoe._id}">
      ...
      <div class="shoeItem--brand">${shoe.brand ? shoe.brand.name : ''}</div>
...
const renderShoeDetails = (shoe) => {
  return `<div id="shoeDetails">
    ...
    <div id="shoeDetails--leftpanel">
      ...
      <div id="shoeDetails--brand">By ${shoe.brand ? shoe.brand.name : ''}</div>
      <div id="shoeDetails--categories">Categories: ${shoe.categories ? shoe.categories.map((c) => c.name).join(', ') : ''}</div>
      <div id="shoeDetails--season">Season: ${shoe.categories ? shoe.categories.map((c) => c.season).join(', ') : ''}</div>
